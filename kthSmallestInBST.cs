...
int kthSmallestInBST(Tree<int> t, int k) {
    ...
    // main loop to traverse the BST
    while (current != null) {
        // since we need to do an inOrderTraversal, we always go left first
        // to find the smallest value. So if left is null, we can start to
        // to look right.
        if (current.left == null) {
            ...
            Tree<int> previous = current.left;
            // traverse until we find an empty right node (or ourself)
            while (previous.right != null && previous.right != current) {
                previous = previous.right;
            }
            
            if (previous.right == null) {
                // set the current as the right node of the previous
                previous.right = current;
                current = current.left;
            } else {
                // clear out the reference to ourself as the right node
                previous.right = null;
                value = current.value;
                current = current.right;
                i++;
