...
int[] nextLarger(int[] a) {
    // NOTE: this solution is actually O(n log n).
    int[] ret = new int[a.Length];
    
    // main iteration loop
    for (int i = a.Length - 1; i >= 0; i--) {
        // the last element will always be -1, as there are no elements afterward
        if (i == a.Length -1) {
            ret[i] = -1;
            continue;
        }
        ...
        int iVal = a[i];
        // initialize the stored value to -1, we will update it later if we find one
        ret[i] = -1;
        // iterate forward in the list to find the next greatest
        for (int x = i + 1; x < a.Length; x++) {
            int xVal = a[x];
            // when we find the greater value, store its value
            if (xVal > iVal) {
                ret[i] = xVal;
                break;
            }
