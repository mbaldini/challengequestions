...
int sumInRange(int[] nums, int[][] queries) {
    // calculate the value for the modulus
    int modValue = (int)Math.Pow(10, 9) + 7;
    
    // array to hold the summed values
    int[] sums = new int[nums.Length + 1];
    
    sums[0] = 0;
    sums[1] = nums[0];
    
    // populate the sums array by iterating through all numbers
    // and adding the result from the previous step to the current
    // number and modding it by modValue
    ...
    // iterate through the queries and calculate the sum of the modded values.
    for(int i = 0; i < queries.Length; i++){
        sum += sums[queries[i][1] + 1] - sums[queries[i][0]];
        sum %= modValue;
    }
    
    return sum > 0 ? sum %= modValue : modValue + sum;
}
