...
string findProfession(int level, int pos) {
    if (level == 1) {
        // always an engineer at level 1
        return "Engineer";
    } else if (findProfession(level - 1, (pos + 1) / 2) == "Engineer") {
        // since the parent is an Engineer, it is an Engineer if the pos is odd
        return pos % 2 == 1 ? "Engineer" : "Doctor";
    } else {
        // since the parent is a Doctor, it is a Doctor if the pos is odd
